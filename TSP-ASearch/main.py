...
def mst_heuristic(graph, path):
    remaining_cities = set(graph.keys()) - set(path)
    ...
    for city1 in remaining_cities:
        for city2 in remaining_cities:
            if city1 != city2:
                G.add_edge(city1, city2, weight=graph[city1][city2])
    mst = nx.minimum_spanning_tree(G)
    mst_cost = mst.size(weight='weight')
    min_edge = min(graph[path[-1]][city] for city in remaining_cities)
    return mst_cost + min_edge
...
def tsp_astar(graph):
    ...
    while pq:
        f, current_city, path, current_cost = heapq.heappop(pq)
        if len(path) == len(cities) and path[0] == start:
            ...
            for next_city in cities:
                if next_city not in path or (len(path) == len(cities) and next_city == start):
                    ...
                    heuristic_cost = new_cost + mst_heuristic(graph, new_path)
                    heapq.heappush(pq, (heuristic_cost, next_city, new_path, new_cost))
...
route, cost = tsp_astar(graph_2)
print(f"Best route (A*): {route} with cost: {cost}")
